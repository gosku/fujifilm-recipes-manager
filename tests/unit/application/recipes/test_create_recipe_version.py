...
_OP = "src.application.usecases.recipes.create_recipe_version.recipe_operations.get_or_create_recipe_from_data"
...
class TestCreateRecipeVersion:
    ...
    def test_raises_already_exists_when_recipe_was_not_created(self) -> None:
        ...
        with patch(_OP, return_value=(recipe, False)):
            with pytest.raises(RecipeAlreadyExistsError) as exc_info:
                create_recipe_version(data=_make_data(), group_id=1)
    ...
    def test_raises_version_line_group_not_found_when_operation_raises_it(self) -> None:
        with patch(
            _OP,
            side_effect=recipe_operations.VersionLineGroupNotFoundError(group_id=99),
        ):
            with pytest.raises(VersionLineGroupNotFoundError) as exc_info:
                create_recipe_version(data=_make_data(), group_id=99)
